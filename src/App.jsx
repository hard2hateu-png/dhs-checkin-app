...
function QRScanner({ onScan, onClose }) {
  ...
  function handleManual() {
    const ticketId = normalizeTicketId(manualInput.trim());
...
function RegistrationForm({ ticketId, initial, onSubmit, onCancel, saving }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#0a0a0a", position: "relative" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "20px 20px 140px" }}>
...
function AttendeeList({ attendees, onSelect, onScanNav, loading }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "20px 20px 0", background: "#0d0d0d", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <button type="button" onClick={onScanNav} style={{ background: "#00ff88", color: "#000", border: "none", borderRadius: 12, padding: "12px 18px", display: "flex", alignItems: "center", gap: 8, fontFamily: "'Space Mono', monospace", fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation" }}>
            <IconScan /> SCAN
          </button>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "0 20px 100px" }}>
        {loading && attendees.length === 0 && (
          <div style={{ textAlign: "center", padding: "48px 0", color: "#666", fontFamily: "'Space Mono', monospace" }}>
            Cargando lista...
          </div>
        )}
        ...
        {!loading && filtered.map((a) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ width: 32, height: 32, background: checked ? "#00ff88" : "#1a1a1a", borderRadius: "50%", border: checked ? "none" : "1px solid #2a2a2a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: "#000", pointerEvents: "none" }}>
                {checked && <IconCheck />}
              </div>
            </button>
          );
...
function AttendeeDetail({ attendee, onCheckIn, onUndoCheckIn, onRegister, onBack, saving }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", position: "relative" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "24px 20px 160px" }}>
        ...
        {checked && (
          <div style={{ background: "#0a1f0a", border: "1px solid #1a3a1a", borderRadius: 16, padding: "18px 20px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <button type="button" onClick={() => onUndoCheckIn(attendee.ticket_id)} disabled={saving} style={{ background: "#1a1a1a", border: "1px solid #333", borderRadius: 8, padding: 8, color: "#ff4444", cursor: "pointer", display: "flex" }} title="Deshacer check-in">
                <IconUndo />
              </button>
            </div>
          </div>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "16px 20px 28px", background: "linear-gradient(transparent, #0a0a0a 40%)" }}>
        {registered && (
          <button type="button" onClick={() => onRegister(attendee.ticket_id)} style={{ width: "100%", background: "#222", color: "#fff", border: "1px solid #333", borderRadius: 16, padding: 16, fontFamily: "'Space Mono', monospace", fontWeight: 700, fontSize: 15, cursor: "pointer", marginBottom: 10, letterSpacing: 0.5 }}>
            EDITAR REGISTRO
          </button>
        )}
...
export default function App() {
  ...
  async function refreshList() {
    ...
    try {
      ...
      // Only show fallback if the sheet/API fails.
  ...
  return (
    <>
      ...
      >
        ...
        {screen === "detail" && ticketLoading && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 16 }}>
            <div style={{ width: 40, height: 40, border: "3px solid #1a1a1a", borderTop: "3px solid #00ff88", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
            <div style={{ color: "#555", fontFamily: "'Space Mono', monospace", fontSize: 13 }}>Cargando ticket...</div>
        ...
        {notFoundMsg && (
          <div style={{ position: "fixed", bottom: 100, left: "50%", transform: "translateX(-50%)", width: "calc(100% - 40px)", maxWidth: 440, background: "#ff4444", color: "#fff", padding: "12px 20px", borderRadius: 12, textAlign: "center", fontFamily: "'Space Mono', monospace", fontSize: 13, zIndex: 1000, boxShadow: "0 10px 30px rgba(0,0,0,0.5)", pointerEvents: "none" }}>
            {notFoundMsg}
          </div>
        )}
      </div>
