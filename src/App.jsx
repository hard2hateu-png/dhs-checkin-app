...
function QRScanner({ onScan, onClose }) {
  ...
  function handleManual() {
    if (manualInput.trim()) {
      onScan(normalizeTicketId(manualInput));
    }
  }
...
function RegistrationDirections() {
  return (
    ...
    >
      ...
      >
        Por favor completa todos los campos requeridos. Verifica que tu información esté correcta antes de enviar.
        <br />
        Solo podrás editar y enviar este registro.
      </div>
    </div>
  );
}
...
function StaffPasswordScreen({ onUnlock }) {
  ...
  function handleSubmit(event) {
    ...
    if (password === STAFF_PASSWORD) {
      sessionStorage.setItem("dhs_staff_unlocked", "true");
      setError("");
      onUnlock();
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", height: "100%", background: "#0a0a0a", padding: "24px 20px", justifyContent: "center" }}>
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 22, fontWeight: 700, color: "#fff", marginBottom: 6 }}>
        ACCESO STAFF
      </div>
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, color: "#666", marginBottom: 18 }}>
        Ingresa la contraseña para ver check-in, scanner y lista de tickets.
      </div>
      ...
      {error && (
        <div style={{ marginTop: 12, color: "#ff7777", fontFamily: "'Space Mono', monospace", fontSize: 12 }}>
          {error}
        </div>
      )}
      ...
      <button
        type="submit"
        style={{ width: "100%", background: "#00ff88", color: "#000", border: "none", borderRadius: 16, padding: 18, fontFamily: "'Space Mono', monospace", fontWeight: 700, fontSize: 16, cursor: "pointer", marginTop: 18, letterSpacing: 0.5 }}
      >
        ENTRAR
      </button>
    </form>
...
export default function App() {
  ...
  const [staffUnlocked, setStaffUnlocked] = useState(() => {
    if (isPublicTicketMode) return false;
    return sessionStorage.getItem("dhs_staff_unlocked") === "true";
  });
  ...
  function handleRegisterStart(ticketId) {
    const cleanId = normalizeTicketId(ticketId);
    if (isPublicTicketMode && cleanId !== urlTicketId) return;
  ...
  async function handleRegisterSubmit(formData) {
    ...
    if (isPublicTicketMode && cleanId !== urlTicketId) {
      showError("Solo puedes editar este ticket");
      return;
    }
    ...
    try {
      const updated = await apiRegisterTicket({
        ...formData,
        ticket_id: isPublicTicketMode ? urlTicketId : cleanId,
      });
      upsertAttendee(updated);

      if (isPublicTicketMode) {
        setScreen("register");
        showError("Registro guardado", 2500);
      } else {
        setScreen("detail");
      }
    } catch (err) {
  ...
  return (
    <>
      ...
      >
        ...
        {showStaffApp && screen === "detail" && selectedAttendee && (
          <AttendeeDetail
        ...
        {(showStaffApp || isPublicTicketMode) && screen === "register" && (
          <RegistrationForm
            ticketId={selectedId}
            initial={selectedAttendee}
            saving={saving}
            publicMode={isPublicTicketMode}
            onSubmit={handleRegisterSubmit}
            onCancel={() => {
              if (!isPublicTicketMode) setScreen("detail");
            }}
          />
        )}
