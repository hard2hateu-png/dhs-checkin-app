...
// FIX A: homepage URL returns “” instead of being parsed as a ticket
...
```
// Check path first (e.g. /DHS26-001)
...
// It's a URL but has no ticket → return "" (prevents homepage from becoming a ticket)
return "";
```
...
// FIX B: read every possible column name; fallback to index-based ID
...
const cleanTicketId =
normalizeTicketId(possibleTicketId) || fallbackTicketId;
...
cosmetology_license:
ticket?.cosmetology_license || ticket?.[“Cosmetology License”] || “”,
...
`${API_URL}?ticket_id=${encodeURIComponent(cleanId)}`
);
if (!data.success) throw new Error(data.error || “Ticket not found”);
return normalizeTicket(data.ticket);
}

// FIX B: pass index so fallback labels are always preserved
...
if (r.includes(“ESTILISTA”))
return { bg: “#1a2f1a”, text: “#4ade80”, label: “ESTILISTA” };
if (r.includes(“PROPIETARIO”))
return { bg: “#1a1f3a”, text: “#60a5fa”, label: “PROPIETARIO” };
if (r.includes(“ESTUDIANTE”))
return { bg: “#2a1a2f”, text: “#c084fc”, label: “ESTUDIANTE” };
...
function scanFrame() {
  ...
  if (video && canvas && video.readyState === video.HAVE_ENOUGH_DATA) {
    ...
    if (code?.data) {
      const ticketId = normalizeTicketId(code.data);
      if (ticketId) {
        stopCamera();
        onScan(ticketId);
        return;
...
const id = normalizeTicketId(manualInput.trim());
if (id) onScan(id);
}
...
type=“button”
onClick={onClose}
style={{ background: “none”, border: “none”, color: “#aaa”, cursor: “pointer”, padding: 4, display: “flex” }}
>
<IconBack />
</button>
<span
...
```
  ...
  >
    ...
    {/* FIX C: pointerEvents none so overlay never blocks touches */}
    <div
      style={{
        position: "absolute",
        inset: 0,
...
```
    {/* NOTE: using onSubmit on the form but buttons are type="button" / type="submit"
        so no accidental form submissions block iOS taps */}
...
```
  ...
  >
    ...
    >
      ...
      <option value="PROPIETARIO DE SALON DE BELLEZA">
        PROPIETARIO DE SALON DE BELLEZA
      </option>
      <option value="ESTUDIANTE">ESTUDIANTE</option>
    </select>

    <FormInput
      label="Licencia de cosmetologia"
      value={form.cosmetology_license}
      onChange={(v) => update("cosmetology_license", v)}
    />
    <FormInput
      label="Telefono"
      value={form.phone}
    ...
    <FormInput
      ...
      onChange={(v) => update("vendor_rep", v)}
    />
  </div>

  {/* FIX C: sticky (not absolute) so it never floats over list cards */}
  <div
    style={{
      position: "sticky",
      bottom: 0,
...
return (
// FIX C: no overflow:hidden on the outer wrapper; let the scroll div handle it
...
{loading
? “Cargando…”
: `${totalChecked} check-ins · ${totalRegistered} registrados · ${attendees.length} tickets`}
</div>
</div>
...
```
    ...
    {/* FIX: progress bar inner div uses height:"100%" not minHeight:"100dvh" */}
    <div
      style={{
        height: 3,
        background: "#1a1a1a",
    ...
    >
      <div
        style={{
          height: "100%",
          width: `${attendees.length ? (totalChecked / attendees.length) * 100 : 0}%`,
          background: "#00ff88",
          borderRadius: 2,
  ...
  {/* FIX C: WebkitOverflowScrolling for iOS momentum scrolling */}
  <div
    style={{
      flex: 1,
      overflowY: "auto",
  ...
  >
    ...
    {filtered.map((a) => {
      ...
      return (
        // FIX C: <button type="button"> with no absolute overlays above it
        <button
          type="button"
          key={a.ticket_id || `row-${Math.random()}`}
          onClick={() => onSelect(a.ticket_id)}
          style={{
            width: "100%",
            background: "#111",
            border: `1px solid ${
              checked ? "#1a3a1a" : registered ? "#252520" : "#2a1a1a"
            }`,
            borderRadius: 14,
            padding: "16px 18px",
            marginBottom: 10,
  ...
  {/* FIX C: logout sits below the list, not as a fixed/absolute overlay */}
  <div
    style={{
...
<div
style={{ fontFamily: “‘DM Sans’, sans-serif”, fontSize: 16, color: “#ddd” }}
>
{value}
</div>
</div>
...
fontFamily: “‘DM Sans’, sans-serif”,
fontSize: 13,
color: checked ? “#00ff88” : registered ? “#fbbf24” : “#ff7777”,
}}
>
{checked
? “✓ Check-in completo”
: registered
? “Registrado · pendiente check-in”
: “No registrado”}
</div>
</div>
</div>
...
```
  ...
  {/* FIX C: sticky not absolute — won't float over other screens */}
  <div
    style={{
      position: "sticky",
      bottom: 0,
  ...
  >
    ...
    {registered && !checked && (
      <button
        type="button"
        disabled={saving}
        onClick={() => onCheckIn(attendee.ticket_id)}
        style={{
          width: "100%",
          background: saving ? "#1a1a1a" : "#00ff88",
...
// FIX D: staff mode read from localStorage; persists across scans
const [isStaff, setIsStaff] = useState(
() => localStorage.getItem(“STAFF_MODE”) === “true”
);
...
() => attendees.find((a) => a.ticket_id === selectedId),
...
if (!exists)
return […prev, clean].sort((a, b) =>
a.ticket_id.localeCompare(b.ticket_id)
);
return prev.map((a) =>
a.ticket_id === clean.ticket_id ? { …a, …clean } : a
);
});
}, []);
...
// FIX E/F: guest goes to “guest” screen; staff goes to “detail”
const handleSelect = useCallback(
async (ticketId) => {
const cleanId = normalizeTicketId(ticketId);
if (!cleanId) return; // FIX A: empty means homepage or invalid URL
...
// FIX B: index passed inside apiListTickets already; set directly
setAttendees(rows);
} catch {
// Keep fallback list
} finally {
...
useEffect(() => {
// FIX A: use normalizeTicketId on the full href; homepage returns “”
const ticketFromUrl = normalizeTicketId(window.location.href);

```
if (ticketFromUrl) {
  // FIX E: guest opens their specific ticket page, can't go to list
...
if (isStaff) {
  ...
  // Non-staff with no ticket URL → show a neutral landing page
  setScreen("list");
}
...
// FIX D: login stores in localStorage; reload so isStaff state re-reads
function loginStaff() {
...
```
  {/* FIX C: root wrapper must NOT have overflow:hidden — that traps iOS taps.
      Use overflowY:auto + overflowX:hidden instead. */}
  <div
    style={{
      background: "#0a0a0a",
      minHeight: "100dvh",
  ...
  >
    {/* FIX G: show "Cargando..." until API ticket data is ready */}
    {ticketLoading && (
      <div
        style={{
          padding: 40,
    ...
    {/* GUEST LANDING (no ticket URL, not staff) */}
    {!ticketLoading && screen === "list" && !isStaff && (
      <div
        style={{
          minHeight: "100dvh",
    ...
    {/* STAFF SCANNER (FIX F: onScan normalizes and goes to detail, not guest) */}
    ...
    {/* REGISTRATION FORM (guest or staff) */}
    ...
    {/* ERROR TOAST — fixed, above content, pointer-events none so list is still tappable */}
    {notFoundMsg && (
      <div
        style={{
          position: "fixed",
